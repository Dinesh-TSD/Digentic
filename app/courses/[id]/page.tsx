    ...
    const fetchCourse = async () => {
      try {
        ...
        if (result.success) {
          ...
          const relatedResponse = await fetch(`/api/courses?category=${result.data.category}&limit=4`);
          const relatedResult = await relatedResponse.json();
          if (relatedResult.success) {
            setRelatedCourses(relatedResult.data.filter((c: Course) => c.id !== result.data.id));
          }
        } else {
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-[#0a0a0a] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-[#666666] dark:text-[#94a3b8]">Loading course...</p>
        </div>
      </div>
    );
  }
